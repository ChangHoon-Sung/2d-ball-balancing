...
tracker = BallTracker()
tracker.start()
pd_ctl = PIDController(kp_x=0.8, ki_x=0.001, kd_x=14, kp_y=0.8, ki_y=0.001, kd_y=14)
...
try:
    while True:
        ...
        print(f"tgt: {target_position[pos_idx]}, pos: {position}, fps: {1/(time()-now):.2f}, out: {output[0]:.2f}, {output[1]:.2f}")
        servo_ctl.set_angle(GPIO_PIN_X, output[0] + OFFSET_X)
        servo_ctl.set_angle(GPIO_PIN_Y, output[1] + OFFSET_Y)
except KeyboardInterrupt:
        servo_ctl.set_angle(GPIO_PIN_X, OFFSET_X)
        servo_ctl.set_angle(GPIO_PIN_Y, OFFSET_Y)
